...
void speed_cntr_Move (int32 step, uint32 accel, uint32 decel, unsigned long speed, speedRampData *srd)
{
  ...
  int T1_FREQ_148 = ((int)((T1_FREQ*0.676)/100)); // divided by 100 and scaled by 0.676
...
void speed_cntr_Update(speedRampData *srd)
{
  ...
  switch (srd->run_state)
    ...
    case ACCEL:
      ...
      srd->accel_count++;
      srd->act_pos += srd->dir;
      new_step_delay = srd->step_delay - (((2 * (long) srd->step_delay) + srd->rest) / (4 * srd->accel_count + 1));
...
void speed_cntr_Init_Timer ()
{
  ...
  Timer_X_Stop ();
  isr_X_StartEx (Timer_X_ISR); // Point to MY_ISR to carry out the interrupt sub-routine
  ...
  Timer_Y_Stop ();
  isr_Y_StartEx (Timer_Y_ISR); // Point to MY_ISR to carry out the interrupt sub-routine
}
